...
func newWorkflows(c *ExtensionsClient, namespace string) *workflows {
...
func (c *workflows) List(opts api.ListOptions) (result *extensions.WorkflowList, err error) {
	...
	err = c.r.Get().Namespace(c.ns).Resource("workflows").VersionedParams(&opts, api.Scheme).Do().Into(result)
	return
}

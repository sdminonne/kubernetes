...
func NewWorkflowController(kubeClient clientset.Interface, resyncPeriod controller.ResyncPeriodFunc) *WorkflowController {
	...
	wc.workflowStore.Store, wc.workflowController = framework.NewInformer(
		&cache.ListWatch{
			ListFunc: func(options api.ListOptions) (runtime.Object, error) {
				return wc.kubeClient.Batch().Workflows(api.NamespaceAll).List(options)
			},
			WatchFunc: func(options api.ListOptions) (watch.Interface, error) {
				return wc.kubeClient.Batch().Workflows(api.NamespaceAll).Watch(options)
			},
		},
		&extensions.Workflow{},
...
func (w *WorkflowController) updateWorkflowStatus(workflow *extensions.Workflow) error {
	_, err := w.kubeClient.Batch().Workflows(workflow.Namespace).UpdateStatus(workflow)
	return err
}

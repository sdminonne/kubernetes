...
func TestAdmissionIgnoresDelete(t *testing.T) {
	...
	handler := NewResourceQuota(kubeClient, install.NewRegistry(kubeClient))
	namespace := "default"
	...
	resourceQuota := &api.ResourceQuota{
		...
		Status: api.ResourceQuotaStatus{
			Hard: api.ResourceList{
				api.ResourceCPU:         resource.MustParse("3"),
				api.ResourceMemory:      resource.MustParse("100Gi"),
				api.ResourceMemoryLimit: resource.MustParse("200Gi"),
				api.ResourcePods:        resource.MustParse("5"),
			},
			Used: api.ResourceList{
				api.ResourceCPU:         resource.MustParse("1"),
				api.ResourceMemory:      resource.MustParse("50Gi"),
				api.ResourceMemoryLimit: resource.MustParse("100Gi"),
				api.ResourcePods:        resource.MustParse("3"),
			},
		},
	}
